...
$.fn.imagesInput = function(options) {
    ...
    function add(plugin,path){
        ...
        var sn = $("#storage_sn").html();
        
        //更新订单图片
        update_images(value_new,sn);
        //触发事件
    ...
    function update_images(value_json,sn){
 		 $.ajax({
              url:"/depot/storage/update_storage_images",
              type:"POST",
              data:{"sn":sn,"upload_images":value_json},
              dataType:"json",
              async:false,
              error: function() {
                  //alert('服务器超时，请稍后再试');
